...
class DashboardView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        vehicles_in_use_pks = Route.objects.filter(start_time__lte=now, end_time__gte=now).values_list('vehicle_id', flat=True)
...
class DriverListView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        stats = {
            ...
            'active': Driver.objects.filter(is_active=True).count(),
            'inactive': Driver.objects.filter(is_active=False).count(),
            'suspended': Driver.objects.filter(is_active=False).count()
        }
        context = { 
            'drivers': queryset, 
            'add_form': DriverForm(),
...
class VehicleListView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        stats = {
            ...
            'on_route': Vehicle.objects.filter(status='on_route').count(),
            'maintenance': Vehicle.objects.filter(status='maintenance').count(),
            'disabled': Vehicle.objects.filter(status='disabled').count(),
        }
        context = {
            'vehicles': queryset, 'add_form': VehicleForm(),
...
class RouteListView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        search_query = request.GET.get('search', '')
        if search_query:
            all_routes = [route for route in all_routes if 
                search_query.lower() in route.start_location.lower() or
                search_query.lower() in route.end_location.lower() or
                (route.driver and search_query.lower() in route.driver.full_name.lower()) or
                (route.vehicle and search_query.lower() in route.vehicle.plate.lower())
            ]
        stats = {
        ...
        for r in Route.objects.all():
            ...
            if status_slug == 'in_progress':
                stats['active'] += 1
            elif status_slug == 'scheduled':
                stats['planned'] += 1
            elif status_slug == 'completed':
                stats['completed'] += 1
            elif status_slug == 'canceled':
                stats['cancelled'] += 1
        
        status_filter = request.GET.get('status', '')
        if status_filter:
            filtered_routes = [
                route for route in all_routes if route.dynamic_status_slug == status_filter
            ]
        else:
